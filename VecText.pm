...
sub configure {
   ...
   if (defined $params{-min_rel_document_frequency} and $params{-min_rel_document_frequency} =~ /^0?\.\d+|1(\.0+)?$/ and $params{-min_rel_document_frequency} > 0 ) {
	...
	$output_original_texts = $params{-output_original_texts} || undef;
	$output_tokens = $params{-output_tokens} || undef;
...
sub process_data_file {
	...
	DOCUMENT: while (<F>) {
		...
		next if $class_position and not $class;
		...
		my @skipped_tokens = @_data[0..$skip_tokens-1] if $output_original_texts and $skip_tokens;
		...
		debug("\r line $.") unless $. % 10;

		unless ($number_of_lines <= 10 or $. % (int ($number_of_lines/10))) {
		...
		for my $text (@pieces) {
			...
			my $orig = $text if $output_original_texts;
...
# specify the structure of a dictionary and modify reading and writing the dictionary
			...
			if (($min_document_frequency and $min_document_frequency > 1 or $max_document_frequency or $df_percentage
				or $min_rel_document_frequency or $max_rel_document_frequency )
					or
			...
			push @texts, [@words];
			push @classes, $class;
			push @original_texts, $orig if $output_original_texts;
			push @all_skipped_tokens, join(' ', @skipped_tokens).' ' if $output_original_texts and $skip_tokens;
...
=cut
	...
	if (not $dictionary_file and
		...
		for my $i (reverse 0..$#texts) {
			unless (@{$texts[$i]}) {
				...
				splice @original_texts, $i, 1 if $output_original_texts; # removing the original text
				splice @all_skipped_tokens, $i, 1 if $output_original_texts and $skip_tokens; # removing skipped tokens
			}
		}
	...
	if ($output_format eq 'ARFF' or $output_format eq 'SPARSE ARFF') {
		...
		open O, ">$encoding", "$output_dir/$output_file.sparse" or die $!;
		...
		if ($class_position) {
			open RL, ">$encoding", "$output_dir/$output_file.sparse.rlabel" or die $!;
			print RL join "\n", @classes;
			close RL;
		}
	...
	for my $i (  @indexes  ) {
		...
		unless ($#texts <= 100 or $i % (int ($#texts/100))) {
			if (defined $pw) {
				# graphical mode
				unless ($$pw_stop) {
					$pw_progress_bar->value($i/$#texts*100);
					$pw_progress_bar->update();
				} else {
