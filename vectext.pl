...
my $output_original_texts = '';
my $original_text_output = $output_also_frame->Checkbutton(
  -text => 'original texts',
  -variable => \$output_original_texts,
)->pack(-side => 'left', 
@padding);
...
$create_dictionary_opt->Subwidget("slistbox")->configure(-height=>4);
...
sub generate_output {
	...
	# setting the output dictionary when 'current' or 'same as input file' is chosen
	if ($output_dir_option eq 'same as input file') {
		$input_file =~ /[^\\\/]+$/;
		$_output_dir = $`;
	}
	...
	if ($param == 2) {
		...
		$cmd .= qq!--output_original_texts ! if $output_original_texts;
		$cmd .= qq!--output_tokens ! if $output_tokens;
